...
Transition = namedtuple(
    'Transition',
    ('state','action','next_state','reward','done','prob_a','constraint')
)
...
def update_policy(
    ...
    log_lamda: nn.Parameter,
    lamda_optimizer : torch.optim.Optimizer,
    ...
    feasb_batch = torch.cat(batch.constraint).float().to(device)
    ...
    td_target = reward_batch.view_as(next_value) + gamma * next_value - torch.inner(log_lamda.exp(), feasb_batch - 1).view_as(next_value)
    ...
    # lamda update
    lamda_loss = (-1) * (log_lamda.exp() * (feasb_batch.detach() - 1)).mean()
    lamda_optimizer.zero_grad()
    lamda_loss.backward()
    lamda_optimizer.step()

    with torch.no_grad():
        log_lamda.clamp_(min=np.log(1e-3), max=np.log(1e2))
...
def evaluate_single_process(env:Environment, ctrl:Dict, objective:Callable, constraint:Callable, log_lamda:torch.Tensor):
    ...
    return objective(state, lamda = log_lamda.detach().cpu().exp().numpy(), discretized = False), constraint(state), state
...
def search_param_space(
    ...
    evals_batches = Parallel(n_jobs=n_proc)(delayed(evaluate_batch)(env, batch, objective, constraint, log_lamda) for batch in batch_ctrls)
    ...
    weighted_l2 = torch.sum(reward * l2) 
    ...
    for i_episode in tqdm(range(num_episode)):
        ...
        f, g = objective(state_new, lamda = log_lamda.detach().exp().numpy(), discretized = False), constraint(state_new, discretized = False)

        reward_list.append(f)
        reward = torch.tensor([f])

        feasb_tensor = torch.from_numpy(g).unsqueeze(0).float()
        ...
        memory.push(state_tensor, action_tensor, next_state_tensor, reward, False, log_probs, feasb_tensor)
        ...
        if i_episode % verbose == 0:
            ...
            lamda = log_lamda.detach().cpu().exp().numpy()
            ...
            print(
                r"| episode:{} | l1:{:.3f} | l2:{:.3f} | l3:{:.3f} | l4:{:.3f} | l5:{:.3f}".format(
                    i_episode + 1,
                    lamda[0],
                    lamda[1],
                    lamda[2],
