...
class Lawson:
    ...
    def compute_n_tau_Q_lower_bound(self, T : float, n : float, B : float, psi : float):
        denominator = (1 / self.Q + self.fc) * self.Q_dt * self.compute_avg_cross_section_v(T) / 4 - self.A_br * (T * 10 ** 3) ** 0.5 - psi * self.A_cyc * B ** 2 * (T * 10 ** 3) / n
        return 3 * T * 10 ** 3 / denominator
    
    def compute_n_tau_T_Q_lower_bound(self, T : float, n : float, B : float, psi : float):
        denominator = (1 / self.Q + self.fc) * self.Q_dt * self.compute_avg_cross_section_v(T) / 4 - self.A_br * (T * 10 ** 3) ** 0.5 - psi * self.A_cyc * B ** 2 * (T * 10 ** 3) / n
        return 3 * T ** 2 * 10 ** 3 / denominator