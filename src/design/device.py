...
from src.design.plot import plot_design_poloidal
from src.config.neutron_info import *
...
class Blanket:
    def __init__(
        ...
        self.slowing_down_cs = slowing_down_cs
    ...
    def compute_lamda_br(self, E: float):
        breeding_cs = self.breeding_cs * np.sqrt(self.E_thres / E)
        lamda_br = 1 / self.density_6 / breeding_cs
        return lamda_br

    def compute_neutron_energy(self, E_input: float, x: float):
        E_output = np.exp(-x / self.lamda_s) * E_input
        return E_output

    def compute_neutron_flux(self, in_flux: float, in_energy: float, x: float):
        alpha_B = (
            1
            / (2 * self.lamda_s * self.density_6 * self.breeding_cs)
            * np.sqrt(in_energy / self.E_thres)
        )
        flux = in_flux * np.exp((1 - np.exp(x / 2 / self.lamda_s)) / alpha_B)
    ...
    def compute_desire_depth(self, in_energy: float, in_flux: float, out_flux: float):
        ...
        depth = (
            self.lamda_s * np.log(1 + alpha_B * np.log(in_flux / out_flux)) + 0.3
        )
        return depth

    def compute_multiplier_effect(self, in_flux: float, x: float):
        flux = in_flux * np.exp(np.log(2) * 0.5 * x / self.lamda_m_pb)
        return flux
...
class Shielding:
    ...
    def compute_neutron_flux(self, in_flux: float, in_energy: float, x: float):
        flux = in_flux * np.exp(-x / self.lamda_s)
        return flux


class VaccumVessel:
    def __init__(
        self, depth: float, max_heat_load: float, max_neutron_flux: float, material: str
    ):
        self.depth = depth
        self.max_heat_load = max_heat_load
        self.max_neutron_flux = max_neutron_flux
...
class TFcoil:
    ...
    def compute_mechanical_thickness(self):
        ...
        alpha_m = (
            self.B0**2
            / mu
            / self.maximum_allowable_stress
            * (
                2 * self.eps_b / (1 + self.eps_b)
                + 0.5 * np.log((1 + self.eps_b) / (1 - self.eps_b))
            )
        )
        c_m = self.R0 * (1 - self.eps_b - np.sqrt((1 - self.eps_b) ** 2 - alpha_m))
        return c_m

    def compute_superconducting_thickness(self):
        mu = PI * 4 * 10 ** (-7)
        alpha_j = 2 * self.B0 / mu / self.R0 / self.maximum_allowable_J
        c_j = self.R0 * (1 - self.eps_b - np.sqrt((1 - self.eps_b) ** 2 - alpha_j))
        return c_j

    def compute_thickness(self):
        return (
            self.compute_mechanical_thickness()
            + self.compute_superconducting_thickness()
        )
...
class Core:
    ...
    def compute_total_thermal_power_density(
        self, p, T
    ):  # including alpha particle energy from blanket
        Ef = 22.4
        sig_v = self.compute_avg_cross_section_v(T)
    ...
    def compute_neutron_flux(self):
        N = self.profile.compute_n_total(64)
        T = self.profile.compute_T_total(64)
        sig_v = self.compute_avg_cross_section_v(T)

        volume = self.Rc * PI * 2 * PI * self.a**2
        surface = 4 * PI * self.Rc * self.a * np.sqrt((1 + self.k**2) / 2)
    ...
    def compute_desire_geometry(self, thermal_efficiency: float, maximum_wall_load: float, electric_power: float):
        ...
        Ra = (
            ...
            * electric_power
            / thermal_efficiency
            / maximum_wall_load
            * np.sqrt(2 / (1 + self.k**2))
        )
        R_desire = np.sqrt(Ra * self.epsilon)
    ...
    def compute_core_volume(self):
        volume = self.Rc * PI * 2 * PI * self.a**2 * self.k
        return volume
...
class Tokamak:
    def __init__(
        ...
        # armour
        self.armour = Shielding(armour_thickness, armour_density, armour_cs, -1.0, maximum_wall_load)
        ...
        self.blanket_thickness = self.blanket.compute_desire_depth(
            in_energy, in_flux, out_flux
        )

        # shield
        self.shield = Shielding(
            shield_depth,
            shield_density,
            shield_cs,
            maximum_heat_load,
        ...
        self.total_thickness = (
            self.coil_thickness
            + self.blanket_thickness
            + self.armour_thickness
            + self.a
            + self.shield_depth
        )

        # update current source
        self.source.update_B0(
            self.B0 * (1 - (self.a + self.blanket_thickness) / self.Rc)
        )
        self.source.update_plasma_frequency(
            self.profile.compute_n(0.8 * self.a, self.a)
        )
    ...
    def update_design(
        ...
        R, a = self.core.compute_desire_geometry(
            self.thermal_efficiency, self.maximum_wall_load, electric_power
        )
        self.Rc = R
        self.a = a
        ...
        in_flux = self.armour.compute_neutron_flux(
            in_flux, in_energy, self.armour_thickness
        )
    ...
    def update_p_avg(self):
        ...
        sig_v = np.array(
            [
                self.core.compute_avg_cross_section_v(T)
                for T in self.profile.compute_T_profile(65)[:-1]
            ]
        )
        ...
        const = (
            16
            * self.electric_power
            / self.core.compute_core_volume()
            / self.thermal_efficiency
            / Ef
        )
        p_avg = (
            self.profile.T_avg * math.sqrt(const / integral) * 1.6 * 10 ** (-19) * 10**3
        )
    ...
    def update_n_avg(self):
        fp = self.profile.compute_p_profile(65)[:-1] / self.profile.p_avg
        fn = self.profile.compute_n_profile(65)[:-1] / self.profile.n_avg
        fT = self.profile.compute_T_profile(65)[:-1] / self.profile.T_avg
        n_avg = (
            sum(fp)
            / sum(fn * fT)
    ...
    def compute_beta(self):
        ...
        beta = 2 * self.profile.p_avg * 4 * PI * 10 ** (-7) / B**2
        return beta

    def compute_TBR(self):
        sig_v = self.core.compute_avg_cross_section_v(
            self.profile.compute_T_total(64)
        )
        Ndt = self.profile.compute_n_total(64)
        ...
        const = (
            self.blanket.breeding_cs * self.blanket.density_6 * 2 * self.blanket.lamda_s
        )

        def _compute_neutron_flux_E(E):
            return in_flux * np.exp(
                (-1)
                * const
                * (
                    np.sqrt(self.blanket.E_thres / E)
                    - np.sqrt(self.blanket.E_thres / En)
                )
            )

        def _compute_neutron_energy(x):
            if self.blanket.density_pn != -1.0:
        ...
        tr_generation_arr = (
            self.blanket.breeding_cs
            * self.blanket.density_6
            * np.sqrt(self.blanket.E_thres / E_arr)
            * _compute_neutron_flux_E(E_arr)
            * multiplier
        )
        tr_generation_arr += (
            20 * 10 ** (-31) * self.blanket.density_7 * _compute_neutron_flux_E(E_arr)
        )

        tr_generation_arr *= (
            np.sqrt((1 + self.k**2) / 2)
            * 2
            * PI
            * (x_arr + self.a + self.armour_thickness)
            * self.Rc
            * PI
            * 2
            * (x_arr[1] - x_arr[0])
    ...
    def compute_fusion_power(self):
        ...
        Sp_rho = np.array(
            [
                self.core.compute_fusion_power_density(p, T)
    ...
    def compute_thermal_power(self):
        ...
        Sp_rho = np.array(
            [
                self.core.compute_total_thermal_power_density(p, T)
                for p, T in zip(p_profile, T_profile)
            ]
        )
    ...
    def compute_alpha_heating_power(self):
        ...
        Sp_rho = np.array(
            [
                self.core.compute_alpha_heating_density(p, T)
                for p, T in zip(p_profile, T_profile)
            ]
        )
    ...
    def compute_radiation_loss_power(self):
        ...
        Sl_rho = np.array(
            [
                self.core.compute_radiation_loss_power_density(n, T, B, psi)
                for n, T in zip(n_profile, T_profile)
            ]
        )
    ...
    def compute_thermal_loss_power(self):
        ...
        St_rho = np.array(
            [
                self.core.compute_thermal_loss_power_density(n, T, tau)
                for n, T in zip(n_profile, T_profile)
            ]
        )
    ...
    def compute_thermal_confinement_time(self):
        ...
        n = self.profile.n_avg * 10 ** (-20)
        B = self.B0 * (1 - (self.a + self.blanket_thickness) / self.Rc)
        P = self.compute_alpha_heating_power() * 10 ** (
            -6
        ) + self.electric_power * 0.1 * 0.5 * 0.8 * 10 ** (-6)
    ...
    def compute_Ip(self):
        ...
        Ip = (
            7.98
            * tau**1.08
            * (Ea / Ef * self.electric_power / self.thermal_efficiency / 10**6) ** 0.74
        )
        Ip /= (
            self.H**1.08
            * self.Rc**1.49
    ...
    def compute_q(self):
        Ip = self.compute_Ip()
        B = self.B0 * (1 - (self.a + self.blanket_thickness) / self.Rc)
        q = 2 * PI * self.a**2 * B * (1 + self.k**2) / 2
        q /= 4 * PI * 10 ** (-7) * self.Rc * Ip * 10**6
        return q
    ...
    def compute_toroidal_current(self, rho: float):
        ...
        Jt_rho = (
            ...
            * (9 / 8)
            * rho**0.25
            * (alpha**2 * (1 - x) * np.exp(alpha * x) / (np.exp(alpha) - 1 - alpha))
        )
        return Jt_rho
    ...
    def compute_Bp_rho(self, rho: float):
        ...
        integral = sum(
            [
                self.compute_toroidal_current(rho_ / a) * 2 * PI * rho_ * drho
                for rho_ in rho_list
            ]
        )
        Bp_rho = integral * mu / 2 / PI / rho
    ...
    def compute_li(self):
        ...
        Bp_max = mu * Ip / 2 / PI / a
        Bp2_avg = (
            sum([2 * rho * drho * self.compute_Bp_rho(rho) ** 2 for rho in rho_list])
            / a**2
        )
    ...
    def compute_NC_bootstrap_fraction(self):
        ...
        Bp = (
            ...
            * (1 / rho)
            * (
                ((1 + alpha - alpha * x) * np.exp(alpha * x) - 1 - alpha)
                / (np.exp(alpha) - 1 - alpha)
            )
            * 10**6
        )
        ...
        JB = (
            -2.44
            * (rho * a_hat / self.Rc) ** 0.5
            * (p_profile / Bp)
            * (1 / n_profile * nprime + 0.055 / T_profile * Tprime)
            / a_hat
        )

        IB = 2 * sum(JB * rho * drho) * self.a**2 * self.k * PI * 10 ** (-6)
        f_NC = IB / Ip
        return f_NC
    ...
    def check_ignition(self):
        ...
        psi = 10 ** (-3)
        n_tau_lower_bound = self.lawson.compute_n_tau_lower_bound(
            T_operation, n, B, psi
        ) * 10 ** (-20)
        n *= 10 ** (-20)
    ...
    def compute_cost_params(self):
        Vb = (
            ...
            * self.Rc
            * (
                (self.a + self.blanket_thickness)
                * (self.a * self.k + self.blanket_thickness)
                - self.k * self.a**2
            )
        )
        Vtf = (
            ...
            * (
                2 * self.Rc
                - 2 * self.a
                - 2 * self.blanket_thickness
                - self.coil_thickness
            )
            * ((1 + self.k) * self.a + 2 * self.blanket_thickness + self.coil_thickness)
        )
    ...
    def compute_Q(self):
        ...
        Pl = (
            n**2 * self.lawson.A_br * (T * 10**3) ** 0.5
            + psi * self.lawson.A_cyc * B**2 * (T * 10**3) * n
        )
    ...
    def print_lawson_criteria(self, filename: str):
        ...
        n_tau = [
            self.lawson.compute_n_tau_lower_bound(t, n, B, psi) * 10 ** (-20) for t in T
        ]
        n_tau_5 = [
            self.lawson.compute_n_tau_Q_lower_bound(t, n, B, psi, 5) * 10 ** (-20)
            for t in T
        ]
        n_tau_Q = [
            self.lawson.compute_n_tau_Q_lower_bound(t, n, B, psi, self.Q) * 10 ** (-20)
            for t in T
        ]
        n_tau_break = [
            self.lawson.compute_n_tau_Q_lower_bound(t, n, B, psi, 1) * 10 ** (-20)
            for t in T
        ]
        n *= 10 ** (-20)
        ...
        ax.scatter(
            ...
            tau_operation * n,
            c="r",
            label="Tokamak design (Q={:.2f})".format(Q_operation),
        )
    ...
    def print_design_configuration(
        self,
        filename: str,
        x_min: float = 0,
        x_max: float = 15,
        ...
        fig, ax = plot_design_poloidal(
            Rc, a, b, d_armour, d_blanket, d_coil, d_shield, x_min, x_max, y_min, y_max
        )
    ...
    def print_info(self, filename: Optional[str] = None):
        ...
        print(
            "| Lawson nTau : {:.3f} , operation nTau: {:.3f} | {}".format(
        ...
        if filename:
            with open(filename, "w") as f:
                ...
                f.write(
                    "\n| Q-parallel : {:.2f} MW-T/m".format(
                        self.compute_parallel_heat_flux()
                    )
                )
    ...
    def print_overall_performance(self, filename: str):
        ...
        for eps in eps_list:
            try:
                self.update_design(
                    ...
                    k_origin,
                    eps,
                    electric_power_origin,
                    T_avg_origin,
                    B_origin,
                    H_origin,
                    self.armour_thickness,
                    self.RF_recirculating_rate,
                )
                result = self.get_design_performance()
        ...
        axes[0].plot(a_list, n_limits, "c", label="$n/n_G$")
        ...
        axes[1].plot(H_list, n_limits, "c", label="$n/n_G$")
        ...
        axes[2].plot(B_list, n_limits, "c", label="$n/n_G$")
        ...
        axes[3].plot(output_list, n_limits, "c", label="$n/n_G$")
        ...
        axes[3].axvline(
            electric_power_origin / 10**6, 0, 1, c = "k", label="Designed tokamak"
        )
