...
def draw_shield(ax:Axes, R : float, a_inner : float, b_inner : float, a_outer : float, b_outer : float):
    ax.add_patch(
        patches.Polygon(
            np.array([
                [R-a_outer*1.0, -b_outer * 0.8], 
...
def draw_vacuum_vessel(ax:Axes, R : float, a_inner : float, b_inner : float, a_outer : float, b_outer : float):
    ax.add_patch(
        patches.Polygon(
            np.array([
                [R-a_outer*1.0, -b_outer * 0.8], 
...
def plot_design_poloidal(
    ...
    a_blanket = a_armour + 0.05
    b_blanket = b_armour + 0.05
    
    a_shield = a_blanket + d_blanket + 0.05
    b_shield = b_blanket + d_blanket + 0.05
    
    a_coil = a_shield + d_shield + 0.05
    b_coil = b_shield + d_shield + 0.05
    
    fig = plt.figure(figsize = (6, 6))
    ax = fig.add_subplot()

    ax = draw_coil(ax, R, a_coil, b_coil, a_coil + d_coil, b_coil + d_coil)
    ax = draw_shield(ax, R, a_shield, b_shield, a_shield + d_shield, b_shield + d_shield)
    ...
    legend_elements = [
        ...
        Line2D([0],[0], color = 'b', lw = 4, label = "Blanket"),
        Line2D([0],[0], color = 'k', lw = 4, label = "Shield"),
        # Line2D([0],[0], color = 'cyan', lw = 4, label = "Vacuum vessel"),
